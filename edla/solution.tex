...
Un algorithme permettant cette association est "bone heat weighting"\cite{baran2007automatic}.
...
\begin{itemize}
	...
	que soit le nombre de sommet proches.
	\item Ils devraient varier de façon homogène avec la surface. Les valeurs de poids ne devraient pas varier fortement si la surface est lisse et inversement si elle est inégale.
	...
	et la distance de la jointure au modèle. 
\end{itemize}
...
Par exemple, pour la main, les doigts ne peuvent pas bouger librement, on peut donc contraindre les degrés de liberté de chaque jointure et même empêcher les translations : toutes les positions de la main pouvant être exprimées par une série de rotation contraintes des jointures du squelette.\newline

%TODO on ne doit pas dire qu'on utilise sa dans notre etat de l'art vu que celui-ci est faire avant
% ATTENTION : à changer si modification de la méthode d'animation
Une autre solution, celle employée dans notre projet, consiste à n'utiliser que des informations de position obtenue par détection des articulations des doigts de la main \textit{via} un périphérique de capture.
