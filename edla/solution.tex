...
Bien que possédant plus d'informations que les images de profondeur (3 canaux au lieu d'un), les images couleurs moins adapté aux calculs 3D.\\
...
Cependant, ça ajoute des contraintes, notemment le fait de devoir porter un gant standardisés.\\
...
Cette carte de profondeur est utilisée par un grand nombre de méthodes pour les problèmes de clustering du corps humain, notemment les SDK de Kinect 1 et Kinect 2 se base principalement dessus pour fournir le squelette \cite{export:145347}.
Les SDK de Kinect1/Kinect2 étant prévus pour le suivi du corps entier, il sont peu performant pour le suivit précis de parties spécifique (comme la main).
L'utilisation directe du SDK semble donc insufisante pour résoudre notre problème. Cependant, on peut aussi récupérer les données brutes fournies par la Kinect 2 et certaines méthodes se contentent d'une image de profondeur pour la détection de la main \cite{export:238453}.
...
récupére les rayons envoyés par projecteur et ensuite réfléchit par les objets de la scène, ce qui permet d'obtenir les informations de profondeur. 
...
quelle que soit la couleur de peau de l'utilisateur \cite{yoo1999fast}. Dès que la ROI de la main est binarisé, il est possible
...
des parties du corps et permet également de détecter les articulations. Dans ce but, cette méthode utilise l'algorithme 
\cite{lepetit2005randomized} qui crée un arbre de décision à partir des données d'entrainement et descend dans celui-ci
...
et le maillage P représente la main de l'utilisateur. ICP fonctionne en quatre étape distincte.
\begin{itemize}
  ...
  \item Effectuer la transformation du nuage de point ayant la plus petite erreur.
  \item Iterer jusqu'à ce qu'on est atteint la condition de fin fixée par l'utilisateur.
\end{itemize} 
\ \\
...
\begin{itemize}
\item Si la surface est paramétrée par un ensemble de points de controle, comme un polyèdre
...
brusques et il est également assez complexe à mettre en place. Il existe d'autre méthode de tracking qui sont
peut-être plus approprié à la conception d'une nouvelle IHM, mais le tracking n'est pas le seul défaut de cette
...
profondeur de la main, il est difficile de déterminer de nombreuses posture de la main. Cette solution était une première approche de la 
